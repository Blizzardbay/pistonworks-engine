	...
	ST_NAMESPACE_SRT
		...
		// Class Members
			Physics_Object::Physics_Object(st::Model* p_model, const b2BodyType& p_type, b2World* p_world, const bool& p_is_fixed, const float& p_friction, const float& p_restitution, const float& p_density, const Object_Type& p_polygon, const bool& p_is_sensor) :
				...
				PW_CALL(m_shape_vertices = pw::co::Memory::Allocate<b2Vec2>(b2Vec2(), p_model->Mesh()->Vertex_Count()), true);
				...
				switch (p_polygon) {
					case Object_Type::POLYGON: {
						...
						// Create shape
						v_shape.Set(m_shape_vertices, TO_INT32(p_model->Mesh()->Vertex_Count()));

						if (TRY_LINE !v_shape.Validate()) {
							SET_ERROR_STATE(PW_B2SHAPE_INVALID);
							SET_ERROR_TYPE(pw::er::Warning_Error(L"pw::st::Physics_Object", L"Given shape is not valid.", ERROR_LINE, __FILEW__, L"Physics_Object"));
							return;
						}
			...
			Physics_Object::~Physics_Object() {
				if (pw::co::Memory::Deallocate<b2Vec2>(m_shape_vertices) == false) {
			...
			void Physics_Object::Set_Size_Px(const b2Vec2& p_size_px) {
				...
				switch (m_body_type) {
					case Object_Type::POLYGON: {
						...
						// Create shape
						v_shape.Set(m_shape_vertices, TO_INT32(m_vertex_count));

						if (TRY_LINE !v_shape.Validate()) {
							SET_ERROR_STATE(PW_B2SHAPE_INVALID);
							SET_ERROR_TYPE(pw::er::Warning_Error(L"pw::st::Physics_Object", L"Given shape is not valid.", ERROR_LINE, __FILEW__, L"Set_Size_Px"));
							return;
						}
						b2FixtureDef v_fixture{};
			...
			void Physics_Object::Set_Size_M(const b2Vec2& p_size_m) {
				...
				switch (m_body_type) {
					case Object_Type::POLYGON: {
						...
						if (TRY_LINE !v_shape.Validate()) {
							SET_ERROR_STATE(PW_B2SHAPE_INVALID);
							SET_ERROR_TYPE(pw::er::Warning_Error(L"pw::st::Physics_Object", L"Given shape is not valid.", ERROR_LINE, __FILEW__, L"Set_Size_M"));
							return;
						}
						b2FixtureDef v_fixture{};
		...
		// Class Members
			...
			Physics_Factory::Physics_Factory(const b2Vec2& p_gravity, const int32_t& p_velocity_it, const int32_t& p_position_it) :
					...
					m_last_added_body{ NULL },
					m_factory_static{}, m_factory_dynamic{} {
				PW_CALL(m_world = pw::co::Memory::Allocate_Args<b2World>(p_gravity), false);
			}
			Physics_Factory::~Physics_Factory() {
				for (auto i = m_factory_static.begin(); i != m_factory_static.end(); i++) {
					if (pw::co::Memory::Deallocate<st::Physics_Object>(*i) == false) {
						if (*i != nullptr) {
							delete *i;
							*i = nullptr;
						}
				...
				for (auto i = m_factory_dynamic.begin(); i != m_factory_dynamic.end(); i++) {
					if (pw::co::Memory::Deallocate<st::Physics_Object>(i->second) == false) {
						if (i->second != nullptr) {
							delete i->second;
							i->second = nullptr;
						}
				...
				if (pw::co::Memory::Deallocate<b2World>(m_world) == false) {
					if (m_world != nullptr) {
						delete m_world;
						m_world = nullptr;
					}
