	...
	ST_NAMESPACE_SRT
		...
		// Class Members
			...
			Animation::Animation(const float p_animation_length, const uint32_t p_frame_count, const uint32_t p_frame_size_x, Vertex_Data* p_vertices, const size_t& p_vertices_count, const bool p_is_async) :
				...
				PW_CALL(m_vertices_default = pw::co::Memory::Allocate<Vertex_Data>(p_vertices_count), true);

				PW_CALL(m_animation_vertices = pw::co::Memory::Allocate<Vertex_Data>(p_vertices_count), false);

				if (m_animation_vertices == nullptr) {
					if (pw::co::Memory::Deallocate<st::Vertex_Data>(m_vertices_default) == false) {
			...
			Animation::~Animation() {
				...
				if (pw::co::Memory::Deallocate<st::Vertex_Data>(m_animation_vertices) == false) {
					if (m_animation_vertices != nullptr) {
				...
				if (p_render_call == false) {
					if (m_is_async == true) {
						if (TRY_LINE m_vertices_default != nullptr) {
							...
							SET_ERROR_TYPE(pw::er::Warning_Error(L"pw::st::Animation", L"Invalid Parameter: vertices_default was nullptr", ERROR_LINE, __FILEW__, L"Change_Frame"));
							return;
						}
					}
				}
			...
			void Animation::Advance_Frame() {
				if (TRY_LINE m_vertices_default != NULL) {
					...
					SET_ERROR_STATE(PW_NULL_PARAMETER_W);
					SET_ERROR_TYPE(pw::er::Warning_Error(L"pw::st::Animation", L"Invalid Parameter: vertices_default was nullptr", ERROR_LINE, __FILEW__, L"Advance_Frame"));
					return;
				}
			}
		...
		// Class Members
			...
			Animation_Structure::Animation_Structure(const std::vector<std::tuple<st::Animation*, st::Texture*>>& p_animations, const std::vector<std::wstring>& p_animation_ids) :
				...
				if (TRY_LINE p_animations.size() == p_animation_ids.size()) {
					...
					SET_ERROR_TYPE(pw::er::Warning_Error(L"pw::st::Animation_Structure", L"Animation count does not equal id count. Every animation must have an id.", ERROR_LINE, __FILEW__, L"Animation_Structure"));
					return;
				}
			}
			Animation_Structure::~Animation_Structure() {
				for (auto i = m_animations.begin(); i != m_animations.end(); i++) {
					if (pw::co::Memory::Deallocate<st::Animation>(std::get<0>(i->second)) == false) {
					...
					if (m_current_access != std::get<1>(i->second)) {
						if (pw::co::Memory::Deallocate<st::Texture>(std::get<1>(i->second)) == false) {
