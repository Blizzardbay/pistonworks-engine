	...
	ST_NAMESPACE_SRT
		...
		// Class Members
			...
			Actor::Actor(st::Model* p_model, st::Texture_Structure* p_multi_texture, const int32_t& p_layer) :
				...
				for (auto i = m_multi_texture->Textures().begin(); i != m_multi_texture->Textures().end(); i++) {
					v_texture_handles.push_back(i->second->Texture_Handle());
				}
				// Check
			...
			Actor::Actor(st::Model* p_model, const std::vector<std::tuple<st::Animation*, st::Texture*>>& p_animations, const std::vector<std::wstring>& p_animation_ids, const int32_t& p_layer) :
				...
				// Make sure to give st::Mesh all of the possible handles to see if the index needs to be stored 
				std::vector<uint64_t> v_texture_handles{};
				for (auto i = m_animations->Animations().begin(); i != m_animations->Animations().end(); i++) {
					v_texture_handles.push_back(std::get<1>(i->second)->Texture_Handle());
				}
				// Check
				pw::st::Mesh::Validate_Multitex(m_model->Render_Index(), v_texture_handles);
			...
			Actor& Actor::operator=(Actor&& p_rhs) noexcept {
				...
				m_body = p_rhs.m_body;
				m_multi_texture = p_rhs.m_multi_texture;
				m_sound_deposit = p_rhs.m_sound_deposit;
				m_is_rendered = p_rhs.m_is_rendered;
				m_render_toggle = p_rhs.m_render_toggle;
				m_s_id = p_rhs.m_s_id;
				m_text = p_rhs.m_text;
				m_current_animation = p_rhs.m_current_animation;
				m_animations = p_rhs.m_animations;
				m_layer = p_rhs.m_layer;
			...
			Actor::~Actor() {
				if (m_is_copy != true) {
					if (pw::co::Memory::Deallocate<st::Model>(m_model) == false) {
						if (m_model != nullptr) {
							delete m_model;
							m_model = nullptr;
						}
					...
					if (pw::co::Memory::Deallocate<st::Texture_Structure>(m_multi_texture) == false) {
						if (m_multi_texture != nullptr) {
							delete m_multi_texture;
							m_multi_texture = nullptr;
						}
					}
					if (pw::co::Memory::Deallocate<st::Sound_Structure>(m_sound_deposit) == false) {
						if (m_sound_deposit != nullptr) {
							delete m_sound_deposit;
							m_sound_deposit = nullptr;
						}
					}
					if (pw::co::Memory::Deallocate<st::Text>(m_text) == false) {
						if (m_text != nullptr) {
							delete m_text;
							m_text = nullptr;
						}
					}
					if (pw::co::Memory::Deallocate<st::Animation_Structure>(m_animations) == false) {
					...
					if (m_animations == nullptr) {
						if (pw::co::Memory::Deallocate<st::Animation>(m_current_animation) == false) {
			...
			void Actor::Play_Sound(std::wstring p_sound_s_id, bool p_reset_play) {
				if (m_sound_deposit != nullptr) {
					if (m_sound_deposit->Sound(std::move(p_sound_s_id)) != nullptr) {
						PW_CALL(m_sound_deposit->Sound(std::move(p_sound_s_id))->Play(p_reset_play), false);
					}
				}
			}
			void Actor::Pause_Sound(std::wstring p_sound_s_id) {
				if (m_sound_deposit != nullptr) {
					if (m_sound_deposit->Sound(std::move(p_sound_s_id)) != nullptr) {
						PW_CALL(m_sound_deposit->Sound(std::move(p_sound_s_id))->Pause(), false);
					}
				}
			}
			void Actor::Stop_Sound(std::wstring p_sound_s_id) {
				if (m_sound_deposit != nullptr) {
					if (m_sound_deposit->Sound(std::move(p_sound_s_id)) != nullptr) {
						PW_CALL(m_sound_deposit->Sound(std::move(p_sound_s_id))->Stop(), false);
					}
				}
			}
			void Actor::Reset_Sound(std::wstring p_sound_s_id) {
				if (m_sound_deposit != nullptr) {
					if (m_sound_deposit->Sound(std::move(p_sound_s_id)) != nullptr) {
						PW_CALL(m_sound_deposit->Sound(std::move(p_sound_s_id))->Reset(), false);
					}
				}
			}
			...
			void Actor::Change_Volume(std::wstring p_sound_s_id, float p_new_volume, bool windows_style) {
				if (m_sound_deposit != nullptr) {
					if (m_sound_deposit->Sound(p_sound_s_id) != nullptr) {
						PW_CALL(m_sound_deposit->Sound(p_sound_s_id)->Set_Volume(p_new_volume, windows_style), false);
					}
				}
			}
			void Actor::Run_Animation() {
				if (m_current_animation != nullptr) {
					PW_CALL(m_current_animation->Change_Frame(), true);
					PW_CALL(m_model->Mesh()->Change_Texture_Data(m_model->Render_Index(), m_current_animation->Vertices()), false);
				}
			}
			void Actor::Advance_Animation() {
				if (m_current_animation != nullptr) {
					PW_CALL(m_current_animation->Advance_Frame(), true);
					PW_CALL(m_model->Mesh()->Change_Texture_Data(m_model->Render_Index(), m_current_animation->Vertices()), false);
				}
			}
				...
				if (m_current_animation != nullptr) {
					if (p_animation_id != L"") {
						if (m_current_animation != m_animations->Animation(p_animation_id.c_str())._Myfirst._Val) {
							...
							PW_CALL(m_model->Set_Texture(m_animations->Animation(p_animation_id.c_str())._Get_rest()._Myfirst._Val), true);
							if (p_reset == true) {
			...
			void Actor::Set_Texture(std::wstring p_s_id) {
				if (m_multi_texture != nullptr) {
					...
					if (m_model->Texture() != v_temp_texture) {
						PW_CALL(m_model->Set_Texture(v_temp_texture), false);
					}
				}
			}
			...
			void Actor::Set_Text(std::wstring p_new_text) {
				if (m_text != nullptr) {
					PW_CALL(m_text->Set_Text(p_new_text), true);
					m_model->Set_Size(m_text->Size());
				}
			}
			void Actor::Set_Text_Color(glm::vec4 p_color) {
				if (m_text != nullptr) {
					PW_CALL(m_text->Set_Text_Color(p_color), false);
				}
			}
		...
		// Scene_Event
			...
			Scene_Event::~Scene_Event() {
				m_model = nullptr;
				if (pw::co::Memory::Deallocate<st::Event_Base>(m_model_event) == false) {
			...
			void Scene_Event::Run_Event() {
				PW_CALL(m_model_event->Trigger_Event(), false);
			}
		...
		// Class Members
			...
			Sub_Scene_Structure::Sub_Scene_Structure(const std::wstring& p_sub_scene_id, const std::vector<st::Actor*>& p_scene_repository, const bool& p_should_render) :
				...
				for (size_t i = 0; i < m_scene_repository.size(); i++) {
					...
					if (v_temp->Layer() >= INT16_MAX / 4) {
						glm::vec2 v_origin = v_temp->Model()->Position();

						v_temp->Model()->Set_Position(st::Camera::Camera_Position());

						v_temp->Model()->Set_Offset(glm::vec2(v_origin.x, v_origin.y - (float)pw::cm::Constant::Window_Height()));

						if (v_temp->Text() != nullptr) {
							v_temp->Text()->Set_Position(st::Camera::Camera_Position());

							v_temp->Text()->Set_Offset(glm::vec3(v_origin.x, v_origin.y - (float)pw::cm::Constant::Window_Height(), 0.0f));
						}
					}
				}
			}
			Sub_Scene_Structure::~Sub_Scene_Structure() {
				for (auto i = m_scene_repository.begin(); i != m_scene_repository.end(); i++) {
					if (pw::co::Memory::Deallocate<st::Actor>(*i) == false) {
						if (*i != nullptr) {
							delete *i;
							*i = nullptr;
						}
		...
		// Class Members
			...
			Game_Scene::~Game_Scene() {
				for (auto i = m_scene_input.begin(); i != m_scene_input.end(); i++) {
					if (pw::co::Memory::Deallocate<pw::co::Input>(i->second) == false) {
						if (i->second != nullptr) {
							delete i->second;
							i->second = nullptr;
						}
					...
					for (auto j = i->second.begin(); j != i->second.end(); j++) {
						for (auto k = j->second.begin(); k != j->second.end(); k++) {
							for (auto l = k->second.begin(); l != k->second.end(); l++) {
								if (pw::co::Memory::Deallocate<st::Scene_Event>(l->second) == false) {
									if (l->second != nullptr) {
										delete l->second;
										l->second = nullptr;
									}
							...
							for (auto l = k->second.begin(); l != k->second.end(); l++) {
								for (auto h = l->second.begin(); h != l->second.end(); h++) {
									for (auto g = h->second.begin(); g != h->second.end(); g++) {
										if (pw::co::Memory::Deallocate<st::Scene_Event>(g->second) == false) {
											if (g->second != nullptr) {
												delete g->second;
												g->second = nullptr;
											}
				...
				for (auto i = m_sub_scene_deposit.begin(); i != m_sub_scene_deposit.end(); i++) {
					if (pw::co::Memory::Deallocate<st::Sub_Scene_Structure>(i->second) == false) {
						if (i->second != nullptr) {
							delete i->second;
							i->second = nullptr;
						}
				...
				if (pw::co::Memory::Deallocate<st::Physics_Factory>(m_scene_physics) == false) {
			...
			void Game_Scene::Pre_Render() {
					...
					if ((*i)->Physics_Object() != nullptr) {
						if ((*i)->Physics_Object()->Body() != nullptr) {
							for (b2ContactEdge* j = (*i)->Physics_Object()->Body()->GetContactList(); j; j = j->next) {
								...
								if (v_temp != nullptr) {
									PW_CALL(Activate_Event((*i)->S_Id(), v_temp->S_Id()), true);
								}
								else {
									// We could just have a event were we collide with
									// anything.
									PW_CALL(Activate_Event((*i)->S_Id(), L"NULL"), true);
								}
							}
						}
					}
				...
				if (m_render_queue.size() > 0) {
						...
						if (v_temp->Animation() != nullptr) {
							if (v_temp->S_Id().empty() != true) {
								if (v_temp->Animation()->End() == true) {
									PW_CALL(Activate_Event(pw::cm::Constant::PW_ANIMATION_END_EVENT, v_temp->S_Id()), true);
								}
								if (v_temp->Animation()->Start() == true) {
									PW_CALL(Activate_Event(pw::cm::Constant::PW_ANIMATION_START_EVENT, v_temp->S_Id()), true);
								}
							}
						}
							...
							if (v_temp->S_Id().empty() != true) {
								for (auto j = v_temp->Sound_Structure()->All_Sounds()->begin(); j != v_temp->Sound_Structure()->All_Sounds()->end(); j++) {
									if (j->second->End() == true) {
										...
										PW_CALL(Activate_Event(pw::cm::Constant::PW_SOUND_END_EVENT, j->first), true);
									}
									if (j->second->Start() == true) {
										PW_CALL(Activate_Event(pw::cm::Constant::PW_SOUND_START_EVENT, j->first), true);
									}
								}
							}
						}
					...
					PW_CALL(Activate_Event(cm::Constant::PW_HOVER_EVENT, 0, 1), true);

					// Make sure after the fact we stop it from 
					// continuing to-do calculations
					if (m_re_render != false) {
						if (m_listener_id == L"NULL") {
							PW_CALL(pw::co::Listener::Change_Bound_Listener(nullptr), false);
						}
						else {
							st::Actor* v_listener = Access_Model(m_listener_id);
							if (v_listener != nullptr) {
								PW_CALL(pw::co::Listener::Create_Listener(std::bind(&st::Model::Calculate_Center, v_listener->Model())), true);
							}
						}
			...
			void Game_Scene::Pause_All_Sounds() {
				for (auto i = m_main_scene_models.begin(); i != m_main_scene_models.end(); i++) {
					PW_CALL((*i)->Pause_All_Sounds(), true);
				}
			}
			void Game_Scene::Stop_All_Sounds() {
				for (auto i = m_main_scene_models.begin(); i != m_main_scene_models.end(); i++) {
					PW_CALL((*i)->Stop_All_Sounds(), true);
				}
			}
			void Game_Scene::Reset_All_Sounds() {
				for (auto i = m_main_scene_models.begin(); i != m_main_scene_models.end(); i++) {
					PW_CALL((*i)->Reset_All_Sounds(), true);
				}
			}
			void Game_Scene::Activate_Event(const PW_EVENT_ID& p_event_type, const PW_BUTTON_CODE& p_button, const PW_STATE& p_state) {
				if (m_current_scene_events != nullptr) {
					if (m_current_scene_events->count(p_event_type) > 0) {
						switch (p_event_type) {
							case cm::Constant::PW_CLICK_EVENT: {
											...
											for (auto j = i->second.at(p_state).begin(); j != i->second.at(p_state).end(); j++) {
												for (auto h = j->second.begin(); h != j->second.end(); h++) {
													if (j->first->Is_Rendered() == true) {
														...
														glm::ivec2 v_point = glm::ivec2(cm::Constant::Mouse_X_Coord(), cm::Constant::Mouse_Y_Coord());
														...
														for (uint64_t k{ 0 }; k < v_mesh->Vertex_Count(); k++) {
														...
														uint64_t l = v_mesh->Vertex_Count() - 1;
														uint64_t v_nvert = v_mesh->Vertex_Count();
														bool c = false;

														for (uint64_t p{ 0 }; p < v_nvert; l = p++) {
														...
														if (h->second->Event()->Trigger() == GLFW_RELEASE) {
															if (c != true) {
																if (h->second->Event()->Play_State() == true) {
																	PW_CALL(h->second->Run_Event(), true);
																	if (m_new_input != true) {
																		continue;
																	}
																	else {
								...
								if (v_found_button != m_current_scene_events->at(p_event_type).end()) {
									...
									if (v_found_state != m_current_scene_events->at(p_event_type).at(p_button).end()) {
										for (auto i = m_current_scene_events->at(p_event_type).at(p_button).at(p_state).rbegin(); i != m_current_scene_events->at(p_event_type).at(p_button).at(p_state).rend(); i++) {
											for (auto j = i->second.begin(); j != i->second.end(); j++) {
												if (i->first == nullptr) {
													PW_CALL(j->second->Run_Event(), true);
													if (m_new_input != true) {
					...
					if (v_found_collider != m_current_scene_collision_events->end()) {
						...
						if (v_found_object != m_current_scene_collision_events->at(p_collider_s_id).end()) {
							for (auto i = m_current_scene_collision_events->at(p_collider_s_id).at(p_object_s_id).begin(); i != m_current_scene_collision_events->at(p_collider_s_id).at(p_object_s_id).end(); i++) {
								PW_CALL(i->second->Run_Event(), true);
								if (m_new_input != true) {
									continue;
								}
								else {
			...
			void Game_Scene::Set_Active_Input(std::wstring p_new_input_s_id) {
				...
				if (v_found_input != m_scene_input.end()) {
					...
					co::Input::Set_Current_Input(*m_current_scene_input);
				}
				else {
					co::Input::Set_Current_Input(nullptr);
				}
			...
			co::Input* Game_Scene::Input() const {
