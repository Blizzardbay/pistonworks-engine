	...
	CO_NAMESPACE_SRT
		...
		// Class Members 
			std::filesystem::path File_Finder::Find_File(const std::wstring& p_directory, std::wstring p_file_name, const std::wstring& p_extension) {
				...
				SET_ERROR_TYPE(pw::er::Warning_Error(L"pw::co::File Loader", p_file_name.insert(0, L"Could not find file "), ERROR_LINE, __FILEW__, L"Find_File"));
			...
			std::filesystem::path File_Finder::Find_File(const std::string& p_directory, std::string p_file_name, const std::string& p_extension) {
				...
				if (v_werror_msg == std::wstring()) {
					...
					MAINTAIN_ERROR_TYPE(L"pw::co::File Loader", L"\"TO_WSTRING\" had an error.", ERROR_LINE, __FILEW__, L"Find_File");
					return std::filesystem::path();
				}
				SET_ERROR_STATE(PW_FILE_NOT_FOUND);
				SET_ERROR_TYPE(pw::er::Warning_Error(L"pw::co::File Loader", v_werror_msg, v_temp_line_number, __FILEW__, L"Find_File"));
				return std::filesystem::path();
			}
			std::filesystem::path File_Finder::Find_File_From_Extension(const std::wstring& p_directory, std::wstring p_extension) {
				...
				SET_ERROR_STATE(PW_FILE_NOT_FOUND);
				SET_ERROR_TYPE(pw::er::Warning_Error(L"pw::co::File Loader", p_extension.insert(0, L"Could not find file "), ERROR_LINE, __FILEW__, L"Find_File_From_Extension"));
				return std::filesystem::path();
			}
			std::filesystem::path File_Finder::Find_File_From_Extension(const std::string& p_directory, std::string p_extension) {
				...
				if (v_werror_msg == std::wstring()) {
					...
					SET_ERROR_STATE(PW_FUNCTION_ERROR);
					MAINTAIN_ERROR_TYPE(L"pw::co::File Loader", L"\"TO_WSTRING\" had an error.", ERROR_LINE, __FILEW__, L"Find_File_From_Extension");
					return std::filesystem::path();
				}
				SET_ERROR_STATE(PW_FILE_NOT_FOUND);
				SET_ERROR_TYPE(pw::er::Warning_Error(L"pw::co::File Loader", v_werror_msg, v_temp_line_number, __FILEW__, L"Find_File_From_Extension"));
				return std::filesystem::path();
			}
		CO_NAMESPACE_END
PW_NAMESPACE_END