...
//////////////////////////////////
	...
	// Class Members
		...
		PW_VOID Engine_Input::Handle_Mouse_Movement(GLFWwindow* window, double mouse_xpos, double mouse_ypos) {
			...
			Engine_Constant::Set_Mouse_Coords((PW_INT)mouse_xpos + TO_INT((ie::Camera::Camera_Position().x / 2.0f) * Engine_Constant::Window_Width()),
				(PW_INT)(Engine_Constant::Window_Height() - (mouse_ypos - TO_INT((ie::Camera::Camera_Position().y / 2.0f) * Engine_Constant::Window_Height()))));
			if (scene_event_function != nullptr) {
				scene_event_function(Engine_Constant::PW_HOVER_EVENT, 0, 0);
			}
		}
		PW_VOID Engine_Input::Handle_Mouse_Button(GLFWwindow* window, int button, int action, int mods) {
			...
			if (action == GLFW_PRESS) {
				if (scene_event_function != nullptr) {
					scene_event_function(Engine_Constant::PW_CLICK_EVENT, button, action);
				}
			}
		}
		...
		PW_VOID Engine_Input::Set_Current_Input(Engine_Input* new_input) {
			current_input = new_input;
		}
		PW_VOID Engine_Input::Set_Refresh_Function(PW_FUNCTION* new_refresh) {
			refresh_function = *new_refresh;
		}
		PW_VOID Engine_Input::Set_Scene_Event_Function(COMPLEX_FUNCTION_3(PW_EVENT_ID, PW_BUTTON_CODE, PW_STATE)* new_scene_event_function) {
