		...
		// Class Members
			...
			void Engine_Control::Init_Engine(const wchar_t* display_name, int32_t display_width, int32_t display_height) {
				...
				//////////////////////////////////
				{
					...
					if (!main_window) {
						er::Engine_Error::PW_Print_Error(L"GLFW", L"Function Error", __LINE__ - 6, __FILE__);
					}
					#ifdef PW_DEBUG_MODE
						else {
							er::Engine_Error::PW_Print_Error(L"GLFW", L"Window Creation ( Complete )", __LINE__ - 6, __FILE__, true);
						}
					#endif // PW_DEBUG_MODE
				...
				//////////////////////////////////
				{
					...
					#ifdef PW_DEBUG_MODE
						std::wstring v_version_msg{ cm::Engine_Constant::To_WString((char*)glGetString(GL_VERSION)) };
