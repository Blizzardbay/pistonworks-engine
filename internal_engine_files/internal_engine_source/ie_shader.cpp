...
namespace pw {
	namespace ie {
		...
		PW_VOID Shader::Update_Matrices(glm::mat4 model, PW_BOOL model_is_colored) {
		...
		PW_VOID Shader::Update_Projection() {
			...
			glUniformMatrix4fv(camera_location, 1, GL_FALSE, glm::value_ptr(ie::Camera::Update_Camera()));
			glUniformMatrix4fv(projection_location, 1, GL_FALSE, glm::value_ptr(ie::Camera::Update_Projection()));
		}
