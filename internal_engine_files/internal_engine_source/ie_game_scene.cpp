	...
	namespace ie {
		...
		void Game_Scene::Render() {
			...
			if (scene_events.size() != 0) {
				for (PW_SIZE i = 0; i < scene_events.size(); i++) {
					scene_events.at(i).Run_Event();
				}
			}
		...
		std::vector<Scene_Model*> Game_Scene::Load_Scene_From_String(PW_STRING scene_str) {
			...
			while (scene_str.empty() != true) {
				...
				if (type == 'd') {
					if (model_color.x != -1.0f) {
						...
						model = Dynamic_Model((Dynamic_Model::Model_Types)model_type, texture, position, rotation, size, model_color);
						if (collidable == true) {
							if (animated == true) {
						...
						Dynamic_Model model = Dynamic_Model();
						model = Dynamic_Model((Dynamic_Model::Model_Types)model_type, texture, position, rotation, size);
						if (collidable == true) {
							if (animated == true) {
								Animation animation = Animation::Animation(
									animation_length, animation_frames, animation_size_x,
					...
					if (model_color.x != -1.0f) {
						Static_Model model = Static_Model();
						model = Static_Model((Static_Model::Model_Types)model_type, texture, position, rotation, size, model_color);
						if (animated == true) {
							Animation animation = Animation::Animation(
								animation_length, animation_frames, animation_size_x,
								model.Model_Mesh()->Vertices(),
						...
						Static_Model model = Static_Model();
						model = Static_Model((Static_Model::Model_Types)model_type, texture, position, rotation, size);
						if (animated == true) {
							Animation animation = Animation::Animation(
								animation_length, animation_frames, animation_size_x,
								model.Model_Mesh()->Vertices(),
